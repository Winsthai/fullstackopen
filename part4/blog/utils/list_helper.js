...
const mostBlogs = (blogs) => {
  ...
  blogs.forEach((x) => {
    map1.has(x.author)
      ? map1.set(x.author, map1.get(x.author) + 1)
      : map1.set(x.author, 1);
  });

  const mostBlogsAuthor = [...map1.entries()].reduce(
    (highestAuthor, currentAuthor) =>
      highestAuthor[1] > currentAuthor[1] ? highestAuthor : currentAuthor,
  );
...
const mostLikes = (blogs) => {
  ...
  blogs.forEach((x) => {
    map1.has(x.author)
      ? map1.set(x.author, map1.get(x.author) + x.likes)
      : map1.set(x.author, x.likes);
  });

  const mostLikesAuthor = [...map1.entries()].reduce(
    (highestAuthor, currentAuthor) =>
      highestAuthor[1] > currentAuthor[1] ? highestAuthor : currentAuthor,
  );
