...
describe("when there are initially some blogs saved", () => {
  ...
  test("verifies that the unique identifier property of the blog posts is named id", async () => {
    ...
    response.body.forEach((blog) =>
      ids.has(blog.id) ? (duplicates = true) : ids.add(blog.id),
    );
  ...
  describe("addition of a new blog", () => {
    test("creates a new blog post successfully after logging in", async () => {
      ...
      const savedBlog = response.body.filter(
    ...
    test("verifies that if the likes property is missing from the request, it will default to the value 0", async () => {
      ...
      const testBlogSaved = response.body.filter(
        (blog) => blog.title === testBlog.title,
      )[0];
  ...
  describe("deletion of a blog", () => {
    test("succeeds given a valid id", async () => {
      ...
      if (
        blogsAfterDeletion.some(
          (blog) => JSON.stringify(blogToDelete) === JSON.stringify(blog),
        )
      ) {
        assert(false);
      } else {
  ...
  describe("updating a blog", () => {
    test("succeeds when given a valid id", async () => {
      ...
      const newBlog = blogsAfterUpdate.filter(
